...
#      produces the same result as the stepwise form (convert to assets via
#      previewAddByAssets, then divide out the price/bonus scaling).
#   3. Debt equivalence: the inline recalculated-debt-ray calculation produces the
#      same result as the stepwise form (convert shares to assets via
#      previewAddByShares, then divide by price * bonus).
#   4. Bounded recalculation: when debt is recalculated (with no liquidation bonus),
...
proveValid(
    s,
    "Collateral shares to liquidate: inline calculation is equivalent to smart inline calculation",
...
proveValid(
    s,
    "Recalculated debt ray to liquidate: inline calculation is equivalent to smart inline calculation",
    recalculatedDebtRayToLiquidated
    == divUp(
        previewAddByShares(
            suppliedShares
            * collateralAssetPrice
        ...
        debtAssetPrice * liquidationBonus,
    ),
)
