...
def unrealizedFeeAmount(
    ...
    return (totalDebtAfter - totalDebtBefore) * liquidityFee
...
feeAmount2 = feeAmount1 + unrealizedFeeAmount(
    drawnShares=drawnShares,
    previousIndex=drawnIndex1,
    drawnIndex=drawnIndex2,
...
totalAddedAssets2 = (
    ...
    - fromBpsUp(feeAmount2)
)

proveValid(
    s,
    "Share price does not decrease from T1 to T2",
    totalAddedAssets1 <= totalAddedAssets2,
)
...
totalAddedAssets3 = (
    ...
    - fromBpsUp(feeAmount3)
)

proveValid(
    s,
    "Share price does not decrease from T2 to T3",
    totalAddedAssets2 <= totalAddedAssets3,
)

# T3: mint fee shares
addedShares = Int("addedShares")
s.add(
    addedShares + VIRTUAL_SHARES <= totalAddedAssets1 + VIRTUAL_ASSETS,
    totalAddedAssets1 + VIRTUAL_ASSETS
    <= (addedShares + VIRTUAL_SHARES) * MAX_SUPPLY_PRICE,
)
totalAddedAssets3Prime = liquiditySwept + totalDebt(
    drawnShares=drawnShares,
    drawnIndex=drawnIndex3,
    realizedPremiumRay=newRealizedPremiumRay,
    premiumShares=premiumShares,
...
addedShares3Prime = addedShares + toAddedSharesDown(
    assets=fromBpsDown(feeAmount3),
    totalAddedAssets=totalAddedAssets3,
    addedShares=addedShares,
)

proveValid(
    s,
    "Minting fee shares does not decrease the share price",
    (totalAddedAssets3 + VIRTUAL_ASSETS) * (addedShares3Prime + VIRTUAL_SHARES)
    <= (totalAddedAssets3Prime + VIRTUAL_ASSETS) * (addedShares + VIRTUAL_SHARES),
)
