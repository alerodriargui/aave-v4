...
contract BorrowIndex_Scenario2Test is LiquidityHubScenarioBaseTest {
  ...
  function initialAssertions(Stage stage) internal override {
    ...
    if (stage == stages[0]) {
      assets[assetId].t_i[0] = hub.getAsset(assetId);
      spokes[0].t_i[0] = hub.getSpoke(assetId, spokes[0].addr);

      // asset
      assertEq(
        assets[assetId].t_i[0].baseBorrowIndex,
        hub.DEFAULT_ASSET_INDEX(),
        't0_i Asset index'
      );
      assertEq(assets[assetId].t_i[0].baseDebt, 0, 't0_i Asset base debt');
      assertEq(
        assets[assetId].t_i[0].lastUpdateTimestamp,
        timeAt(stages[0]),
        't0_i Asset lastUpdateTimestamp'
      );
    } else if (stage == stages[1]) {
      assets[assetId].t_i[1] = hub.getAsset(assetId);
      spokes[0].t_i[1] = hub.getSpoke(assetId, spokes[0].addr);

      // asset
      assertEq(
        assets[assetId].t_i[1].baseBorrowIndex,
        hub.DEFAULT_ASSET_INDEX(),
        't1_i Asset index'
      );
      assertEq(assets[assetId].t_i[1].baseDebt, 0, 't1_i Asset base debt');
      assertEq(
        assets[assetId].t_i[1].lastUpdateTimestamp,
        timeAt(stages[0]),
      ...
      assets[assetId].t_i[2] = hub.getAsset(assetId);
      spokes[0].t_i[2] = hub.getSpoke(assetId, spokes[0].addr);
      spokes[3].t_i[2] = hub.getSpoke(assetId, spokes[3].addr);

      // asset
      assertEq(
        assets[assetId].t_i[2].baseBorrowIndex,
        hub.DEFAULT_ASSET_INDEX(),
        't2_i Asset index'
      );
      assertEq(
        assets[assetId].t_i[2].baseDebt,
        spokes[0].amounts.draw.t_i[1],
        't2_i Asset base debt'
      );
      assertEq(
        assets[assetId].t_i[2].lastUpdateTimestamp,
        timeAt(stages[1]),
      ...
      assertEq(spokes[0].t_i[3].baseDebt, spokes[0].amounts.draw.t_i[1], 't3_i Spoke1 base debt');
      assertEq(
      ...
      assertEq(spokes[3].t_i[3].baseDebt, spokes[3].amounts.draw.t_i[2], 't3_i Spoke4 base debt');
      assertEq(
  ...
  function exec(Stage stage) internal override {
    ...
    if (stage == stages[1]) {
      Utils.supply({
        ...
        assetId: assetId,
        spoke: spokes[0].addr,
        amount: spokes[0].amounts.supply.t_i[1],
        riskPremium: 0,
        user: bob,
        to: spokes[0].addr
      });
      Utils.draw({
        ...
        assetId: assetId,
        spoke: spokes[0].addr,
        amount: spokes[0].amounts.draw.t_i[1],
        riskPremium: 0,
        to: bob,
      ...
      Utils.draw({
        ...
        assetId: assetId,
        spoke: spokes[3].addr,
        amount: spokes[3].amounts.draw.t_i[2],
        riskPremium: 0,
        to: bob,
      ...
      Utils.supply({
        ...
        assetId: assetId,
        spoke: spokes[3].addr,
        amount: spokes[3].amounts.supply.t_i[3],
        riskPremium: 0,
        user: bob,
        to: spokes[3].addr
      });
  ...
  function finalAssertions(Stage stage) internal override {
    ...
    if (stage == stages[0]) {
      ...
      assertEq(spokes[0].t_f[1].baseDebt, spokes[0].amounts.draw.t_i[1], 't1_f Spoke1 base debt');
      assertEq(
      ...
      assertEq(
        assets[assetId].t_f[2].baseBorrowIndex,
        assets[assetId].t_f[1].baseBorrowIndex.rayMul(states.cumulatedBaseInterest.t_f[2]),
        't2_f Asset index'
      );
      assertEq(
        assets[assetId].t_f[2].baseDebt,
        assets[assetId].t_f[1].baseDebt.rayMul(states.cumulatedBaseInterest.t_f[2]) +
          spokes[3].amounts.draw.t_i[2],
      ...
      assertEq(spokes[3].t_f[1].baseDebt, spokes[3].amounts.draw.t_i[1], 't2_f Spoke4 base debt');
      assertEq(
