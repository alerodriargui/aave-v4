...
contract DeployValidation is Test {
  ...
  address internal _giverPm;
  address internal _takerPm;
  ...
  function setUp() public {
    ...
    _giverPm = _deploy.giverPositionManager();
    _takerPm = _deploy.takerPositionManager();
  ...
  function test_hubAssets() public view {
    for (uint256 i; _config.assetExists(i); i++) {
      ...
      assertEq(
        a.decimals,
        IERC20Metadata(tokenAddr).decimals(),
        string.concat(label, ': decimals')
      );
      ...
      assertEq(
        irData.baseDrawnRate,
        asset.irData.baseDrawnRate,
        string.concat(label, ': baseDrawnRate')
      );
      assertEq(
        irData.rateGrowthBeforeOptimal,
        asset.irData.rateGrowthBeforeOptimal,
        string.concat(label, ': rateGrowthBeforeOptimal')
      );
      assertEq(
        irData.rateGrowthAfterOptimal,
        asset.irData.rateGrowthAfterOptimal,
        string.concat(label, ': rateGrowthAfterOptimal')
      );
    }
  }
  ...
  function test_treasurySpokeRegistrations() public view {
    for (uint256 hi; _config.hubExists(hi); hi++) {
      ...
      for (uint256 assetId; assetId < assetCount; assetId++) {
        ...
        assertTrue(hub.isSpokeListed(assetId, treasuryAddr), string.concat(label, ': not listed'));
  ...
  function test_positionManagers() public view {
    address[5] memory pms = [_sigGateway, _nativeGateway, _giverPm, _takerPm, _configPm];
    bool[5] memory enabled = [
      _config.deploySignatureGateway(),
      _config.deployNativeTokenGateway(),
      _config.deployGiverPositionManager(),
      _config.deployTakerPositionManager(),
  ...
  function test_accessControlConfiguratorSelectors() public view {
    ...
    // SpokeConfigurator: 24 selectors → SPOKE_CONFIGURATOR_ROLE
    bytes4[24] memory spokeConfigSelectors = [
  ...
  function test_tokenizationSpokes() public view {
    for (uint256 i; _config.assetExists(i); i++) {
      ...
      // Underlying asset (IERC4626.asset())
      assertEq(
        ITokenizationSpoke(tsAddr).asset(),
        tokenAddr,
        string.concat(label, ': underlying asset')
      );

      // Decimals match underlying
      assertEq(
        IERC20Metadata(tsAddr).decimals(),
        IERC20Metadata(tokenAddr).decimals(),
        string.concat(label, ': decimals')
      );

      // MAX_ALLOWED_SPOKE_CAP matches hub
      assertEq(
        ITokenizationSpoke(tsAddr).MAX_ALLOWED_SPOKE_CAP(),
        IHub(hubAddr).MAX_ALLOWED_SPOKE_CAP(),
        string.concat(label, ': MAX_ALLOWED_SPOKE_CAP')
      );
    }
  }

  // ==================== Test: Treasury Spoke Ownership ====================

  function test_treasurySpokeOwnership() public view {
    for (uint256 hi; _config.hubExists(hi); hi++) {
      string memory hubKey = _config.hubKey(hi);
      address treasuryAddr = _deploy.treasury(hubKey);
      assertEq(Ownable(treasuryAddr).owner(), _admin, string.concat(hubKey, ': treasury owner'));
    }
  }

  // ==================== Test: IR Strategy Hub Reference ====================

  function test_irStrategyHubReference() public view {
    for (uint256 hi; _config.hubExists(hi); hi++) {
      string memory hubKey = _config.hubKey(hi);
      address hubAddr = _deploy.hub(hubKey);
      address irStratAddr = _deploy.irStrategy(hubKey);
      assertEq(
        IAssetInterestRateStrategy(irStratAddr).HUB(),
        hubAddr,
        string.concat(hubKey, ': irStrategy.HUB')
      );
    }
  }
  ...
  function test_hubAssetCounts() public view {
    for (uint256 hi; _config.hubExists(hi); hi++) {
      ...
      assertEq(hub.getAssetCount(), expectedCount, string.concat(hubKey, ': asset count'));
    }
  }
  ...
  function test_reserveCountsPerSpoke() public view {
    for (uint256 si; _config.spokeExists(si); si++) {
      ...
      assertEq(spoke.getReserveCount(), expectedCount, string.concat(spokeKey, ': reserve count'));
    }
  }
  ...
  function test_authority() public view {
    ...
    // Configurators
    assertEq(IAccessManaged(_hubConfiguratorAddr).authority(), am, 'hubConfigurator: authority');
    assertEq(
