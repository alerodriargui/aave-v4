...
contract TreasurySpokeTest is SpokeBase {
  ...
  function test_withdraw_fuzz_maxLiquidityFee(
    ...
    skip(skipTime);
    assertEq(hub1.getAsset(assetId).pendingFeeShares, 0, 'fees'); // fees not yet accrued
    ...
    assertEq(hub1.getAsset(assetId).pendingFeeShares, 0, 'realized fees after minting');
    assertApproxEqAbs(
