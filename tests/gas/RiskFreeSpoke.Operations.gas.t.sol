...
contract RiskFreeSpokeOperations_Gas_Tests is SpokeBase {
  string internal NAMESPACE = 'RiskFreeSpoke.Operations';
  ...
  /// @dev Override deployFixtures to deploy RiskFreeSpoke instead of regular Spoke
  function deployFixtures() internal virtual override {
    ...
    // Deploy RiskFreeSpoke instances instead of regular Spoke
    (spoke1, oracle1) = _deployRiskFreeSpokeWithOracle(ADMIN, address(accessManager), 'RiskFreeSpoke 1 (USD)');
    (spoke2, oracle2) = _deployRiskFreeSpokeWithOracle(ADMIN, address(accessManager), 'RiskFreeSpoke 2 (USD)');
    (spoke3, oracle3) = _deployRiskFreeSpokeWithOracle(ADMIN, address(accessManager), 'RiskFreeSpoke 3 (USD)');
    treasurySpoke = ITreasurySpoke(new TreasurySpoke(TREASURY_ADMIN, address(hub1)));
    vm.stopPrank();

    vm.label(address(spoke1), 'riskFreeSpoke1');
    vm.label(address(spoke2), 'riskFreeSpoke2');
    vm.label(address(spoke3), 'riskFreeSpoke3');
  ...
  function test_updateRiskPremium() public {
    ...
    // RiskFreeSpoke updateUserRiskPremium is a no-op, but we still snapshot for comparison
    spoke.updateUserRiskPremium(alice);
    vm.snapshotGasLastCall(NAMESPACE, 'updateUserRiskPremium: 1 borrow (no-op)');

    spoke.borrow(reserveId.usdx, 500e6, alice);
    skip(100);

    spoke.updateUserRiskPremium(alice);
    vm.snapshotGasLastCall(NAMESPACE, 'updateUserRiskPremium: 2 borrows (no-op)');
    vm.stopPrank();
  }
  ...
  function _liquidationSetup() internal {
    ...
    // RiskFreeSpoke should always have zero risk premium
    assertEq(userAccountData.riskPremium, 0, 'risk premium should be 0 for RiskFreeSpoke');
