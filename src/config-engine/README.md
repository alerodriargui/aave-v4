...
- **Spoke config** (`HubEngine.executeHubSpokeConfigUpdates`) — dispatches caps, risk premium threshold, and status updates from a single `SpokeConfigUpdate` struct. Calls `updateSpokeCaps`, `updateSpokeSupplyCap`, or `updateSpokeDrawCap` depending on which caps are modified. Updates active/halted flags independently.
