...
library LibSort {
    ...
    assembly {
      ...
      function sortInner(w_, l_, h_) {
        ...
        if iszero(gt(sub(h_, l_), 0x180)) {
          ...
          let i_ := add(l_, 0x20)
          if iszero(lt(mload(l_), mload(i_))) {
            mswap(i_, l_)
          }
          for {
        ...
        // Median of 3 with sorting.
        {
          let e0_ := mload(l_)
          let e1_ := mload(p_)
          if iszero(lt(e0_, e1_)) {
            e0_, e1_ := swap(e0_, e1_)
          }
          let e2_ := mload(h_)
        ...
        if iszero(eq(add(p_, 0x20), h_)) {
          sortInner(w_, add(p_, 0x20), h_)
        }
        if iszero(eq(p_, l_)) {
          sortInner(w_, l_, p_)
        }
      ...
      for {
        ...
        } iszero(gt(mload(add(w, j)), mload(j))) {
        ...
        for {
          j := h
        } iszero(lt(mload(add(w, j)), mload(j))) {
    ...
    assembly {
      ...
      if iszero(lt(mload(a), 2)) {
        ...
        for {
          ...
          if iszero(mul(result, xor(a, end))) {
            break
          }
        }
      }
  ...
  function _searchSorted(
    ...
    assembly {
      ...
      let t := 0
      for {
        needle := add(signed, needle)
      } 1 {

      } {
        index := shr(1, add(l, h))
        t := add(signed, mload(add(a, shl(5, index))))
        if or(gt(l, h), eq(t, needle)) {
          break
        }
