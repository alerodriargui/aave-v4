...
import {RiskFreeUserPositionDebt} from 'src/spoke/libraries/RiskFreeUserPositionDebt.sol';
import {ReserveFlags, ReserveFlagsMap} from 'src/spoke/libraries/ReserveFlagsMap.sol';
import {IHubBase} from 'src/hub/interfaces/IHubBase.sol';
import {IAaveOracle} from 'src/spoke/interfaces/IAaveOracle.sol';
import {ISpoke, ISpokeBase} from 'src/spoke/interfaces/ISpoke.sol';

/// @title RiskFreeLiquidationLogic library
/// @author Aave Labs
/// @notice Implements the logic for liquidations without premium calculations.
library RiskFreeLiquidationLogic {
  ...
  using RiskFreeUserPositionDebt for ISpoke.UserPosition;
  ...
  struct LiquidateUserParams {
    ...
    uint256 drawnDebt;
    // No premiumDebtRay field - risk-free spoke has no premium
    uint256 drawnIndex;
  ...
  /// @notice Liquidates a user position without premium calculations.
  ...
  function liquidateUser(
    ...
    _validateLiquidationCall(
      ValidateLiquidationCallParams({
        ...
        debtReserveBalance: params.drawnDebt, // No premium - just drawn debt
    ...
    (uint256 drawnSharesToLiquidate, bool isDebtPositionEmpty) = _liquidateDebt(
      debtReserve,
    ...
    // Emit with zero premium delta
    IHubBase.PremiumDelta memory zeroPremiumDelta;
  ...
  function _liquidateDebt(
    ...
    // All debt is drawn debt - no premium
    uint256 drawnSharesLiquidated = params.debtToLiquidate.rayDivDown(params.drawnIndex);

    // Zero premium delta - risk-free spoke has no premium
    IHubBase.PremiumDelta memory zeroPremiumDelta;
    ...
    debtReserve.hub.restore(debtReserve.assetId, params.debtToLiquidate, zeroPremiumDelta);

    // No applyPremiumDelta call - risk-free spoke has no premium
    ...
    if (debtPosition.drawnShares == 0) {
      ...
      return (drawnSharesLiquidated, true);
    }

    return (drawnSharesLiquidated, false);
  }
