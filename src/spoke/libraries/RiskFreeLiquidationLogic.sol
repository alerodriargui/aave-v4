...
library RiskFreeLiquidationLogic {
  using SafeCast for *;
  ...
  // see IRiskFreeSpoke.HEALTH_FACTOR_LIQUIDATION_THRESHOLD docs
  uint64 public constant HEALTH_FACTOR_LIQUIDATION_THRESHOLD = 1e18;

  // see IRiskFreeSpoke.DUST_LIQUIDATION_THRESHOLD docs
  ...
  function liquidateUser(
    IRiskFreeSpoke.Reserve storage collateralReserve,
    IRiskFreeSpoke.Reserve storage debtReserve,
    IRiskFreeSpoke.UserPosition storage collateralPosition,
    IRiskFreeSpoke.UserPosition storage debtPosition,
    ISpoke.PositionStatus storage positionStatus,
    IRiskFreeSpoke.LiquidationConfig storage liquidationConfig,
    ...
    _validateLiquidationCall(
      ValidateLiquidationCallParams({
        ...
        debtReserveBalance: params.drawnDebt
      })
    );
  ...
  function _validateLiquidationCall(ValidateLiquidationCallParams memory params) internal pure {
    require(params.user != params.liquidator, IRiskFreeSpoke.SelfLiquidation());
    require(params.debtToCover > 0, IRiskFreeSpoke.InvalidDebtToCover());
    require(
      params.collateralReserveHub != address(0) && params.debtReserveHub != address(0),
      IRiskFreeSpoke.ReserveNotListed()
    );
    require(
      !params.collateralReservePaused && !params.debtReservePaused,
      IRiskFreeSpoke.ReservePaused()
    );
    require(
      params.healthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD,
      IRiskFreeSpoke.HealthFactorNotBelowThreshold()
    );
    require(
      params.isUsingAsCollateral && params.collateralFactor > 0,
      IRiskFreeSpoke.CollateralCannotBeLiquidated()
    );
    require(params.collateralReserveBalance > 0, IRiskFreeSpoke.ReserveNotSupplied());
    require(params.debtReserveBalance > 0, IRiskFreeSpoke.ReserveNotBorrowed());
  }
  ...
  function _calculateLiquidationAmounts(
    ...
    require(params.debtToCover >= debtToLiquidate, IRiskFreeSpoke.MustNotLeaveDust());
  ...
  function _liquidateDebt(
    IRiskFreeSpoke.Reserve storage reserve,
    IRiskFreeSpoke.UserPosition storage position,
    ISpoke.PositionStatus storage positionStatus,
