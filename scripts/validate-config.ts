...
export function validate(raw: unknown): ValidationResult {
  ...
  config.reserves.forEach((r, i) => {
    ...
    if (r.maxLiquidationBonus !== undefined && r.maxLiquidationBonus < 10000) {
      error(
        'E11',
        `${label}: maxLiquidationBonus ${r.maxLiquidationBonus} < 10000 (must be >= 100%)`,
      );
    }

    if (
      r.collateralFactor > 0 &&
      r.maxLiquidationBonus !== undefined &&
      r.maxLiquidationBonus > 0
    ) {
      ...
      if (percentMulUp >= 10000) {
        error(
          'E12',
          `${label}: percentMulUp(maxLiquidationBonus=${r.maxLiquidationBonus}, collateralFactor=${r.collateralFactor}) = ${percentMulUp} >= 10000`,
        );
      }
    }
  ...
  for (const tokenKey of tokensUsedInReserves) {
    ...
    if (token && token.priceFeed === ZERO_ADDRESS) {
      warn(
        'W7',
        `Token "${tokenKey}" has mock price feed (${ZERO_ADDRESS}) but is used in reserves`,
      );
    }
  }
