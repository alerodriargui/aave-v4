...
const tokenizableAssets = config.assets.filter(
  (a) => a.tokenize && a.tokenize.name && a.tokenize.symbol,
);

// Group by hubKey preserving order
function groupByHub(assets) {
  const groups = [];
  const groupMap = {};
  ...
  for (const group of hubGroups) {
    ...
    for (let i = 0; i < assets.length; i++) {
      ...
      if (isLast) {
        ...
        lines.push(
          `    vm.serializeAddress("${hubKey}", "${sanitizeKey(a.tokenKey)}", proxy);`,
        );
      }
    }
  }
...
function genTokenizationPayload(getAddress) {
...
function wrapDeployScript(contractName, body) {
...
function wrapPayloadContract(contractName, body) {
...
(async () => {
  ...
  if (generatePayload) {
    await writeFormatted(
      ...
      wrapPayloadContract(
        "TokenizationSpokePayload",
        genTokenizationPayload(getAddress),
      ),
    );
  }
