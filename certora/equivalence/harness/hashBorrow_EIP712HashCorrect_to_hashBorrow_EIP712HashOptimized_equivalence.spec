...
rule equivalence_of_revert_conditions()
{
    ...
    bool hashBorrow_EIP712HashCorrect_revert;
    bool hashBorrow_EIP712HashOptimized_revert;
    // using this as opposed to generating input parameters is experimental
    env e_hashBorrow_EIP712HashCorrect; calldataarg args;
    env e_hashBorrow_EIP712HashOptimized;
    e_equivalence(e_hashBorrow_EIP712HashCorrect, e_hashBorrow_EIP712HashOptimized);

    hashBorrow@withrevert(e_hashBorrow_EIP712HashCorrect, args);
    hashBorrow_EIP712HashCorrect_revert = lastReverted;

    B.hashBorrow@withrevert(e_hashBorrow_EIP712HashOptimized, args) at init;
    hashBorrow_EIP712HashOptimized_revert = lastReverted;

    assert(hashBorrow_EIP712HashCorrect_revert == hashBorrow_EIP712HashOptimized_revert);
}
...
rule equivalence_of_return_value()
{
    storage init = lastStorage;
    assume_equivalent_states();
    bytes32 hashBorrow_EIP712HashCorrect_bytes32_out0;
    bytes32 hashBorrow_EIP712HashOptimized_bytes32_out0;
    ...
    hashBorrow_EIP712HashCorrect_bytes32_out0 = hashBorrow(e_hashBorrow_EIP712HashCorrect, args);
    hashBorrow_EIP712HashOptimized_bytes32_out0 = B.hashBorrow(e_hashBorrow_EIP712HashOptimized, args) at init;

    assert(hashBorrow_EIP712HashCorrect_bytes32_out0 == hashBorrow_EIP712HashOptimized_bytes32_out0);
}
